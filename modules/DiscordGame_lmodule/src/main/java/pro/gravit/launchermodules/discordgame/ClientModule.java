...
public class ClientModule extends LauncherModule {
    ...
    public static ScopeConfig serverScopeConfig;
    ...
    public void init(LauncherInitContext initContext) {
        config = new Config();
        if (!config.enable || config.scopes == null) return;
        ...
        // "server" опційний: старі конфіги без цієї секції просто не отримають окремого
        // статусу під час перегляду конкретного сервера (тихо ігноруємо відсутність).
        if (config.scopes.get("server") != null) {
            serverScopeConfig = new ScopeConfig(config.scopes.get("server"));
        }
